...
/**
 * Initialize header navigation click handler
 */
function initializeHeaderNavigation() {
  const headerLeft = document.querySelector('.homepage-header-left');
  if (!headerLeft) return;

  headerLeft.addEventListener('click', (e) => {
    // Don't navigate if clicking on search input or explore button
    if (e.target.closest('.homepage-search-container, .explore-wrapper')) {
      return;
    }
    const url = headerLeft.dataset.href;
    if (url) {
