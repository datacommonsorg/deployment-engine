...
# DB data pre-filling is needed for first app load.
python3 upload_db_configs.py

# There is a need to download the mcf parser
pip install requests
python3 download_mcf_parser.py

# Triggering the db-sync job before app start
cd import
pip install setuptools=="80.9.0"
python3 -m stats.main --mode=customdc --input_dir=$INPUT_DIR --output_dir=$OUTPUT_DIR
cd ../
