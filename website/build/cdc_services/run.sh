...
# DB data pre-filling to cloud storage is needed for first app load.
python3 upload_db_configs.py

# Download the mcf parser for import inside container
python3 download_mcf_parser.py

# Run the db-sync and embeddings build
echo "Running db-sync and embeddings build..."
sh run_db_embeddings.sh
echo "DB-sync and embeddings build completed."
...
if [[ $ENABLE_MCP == "true" ]]; then
    ...
    (
      ...
      exec datacommons-mcp serve http --skip-api-key-validation --host 0.0.0.0 --port 8082
    ) &
fi
